...
class TestTracker : public ::testing::Test {
 public:
  TestTracker() : tracker_params_(), tracker_(nullptr) {
    ...
    if (FLAGS_display) {
      ...
      window_->setBackgroundColor(ColorUtils::black());
    }
  }
 ...
 protected:
  ...
  void visualizePointCloud(const std::string& id, const cv::Mat& pointcloud) {
    CHECK(window_);
    cv::viz::WCloud cloud(pointcloud, cv::viz::Color::red()); //nuance w overloaded contructor - TBD
  ...
  void visualizeScene(const CameraParams& camera_1_params,
    ...
    if (FLAGS_display) {
      ...
      drawPixelsOnImg(cam_1_kpts, cam_1_img, ColorUtils::brown(), 6u, 225u);

      // Color image 2 with pixel reprojections
      drawPixelsOnImg(cam_2_kpts, cam_2_img, ColorUtils::green(), 3u, 125u);
      ...
      cv::viz::WCameraPosition cpw_1_frustum(
          K_1, cam_1_img, 1.0, ColorUtils::red());
      cv::viz::WCameraPosition cpw_2_frustum(
          K_2, cam_2_img, 1.0, ColorUtils::green());
